 ...
 */
export function cleanAndSplitText(text) {
    if (!text)
        return [];
    let processed = text;
...
export function normalizeLatexMathDelimiters(text) {
    if (!text)
        return text;
...
export function splitTextByLatexMath(text) {
    if (!text)
        return [];
...
function simplifyLatexBody(mathText) {
...
export function stripLatexMathForDocx(text) {
    if (!text)
        return text;
