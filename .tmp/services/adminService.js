 ...
 */
export const uploadResourceFile = async ({ file, fileType, title, description = '', gradeLevel = '', subject = '', tags = [], isPublic = false, }) => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
 ...
 */
export const getAllResourceFiles = async (fileType) => {
    const isAdmin = await checkIsAdmin();
    if (!isAdmin) {
        throw new Error('Unauthorized: Admin access required');
    }
 ...
 */
export const getPublicResourceFiles = async (fileType) => {
    let query = supabase
        .from('resource_files')
        .select('*')
        .eq('is_public', true)
 ...
 */
export const deleteResourceFile = async (fileId) => {
    const isAdmin = await checkIsAdmin();
    if (!isAdmin) {
        throw new Error('Unauthorized: Admin access required');
    }
 ...
 */
export const updateResourceFile = async (fileId, updates) => {
    const isAdmin = await checkIsAdmin();
    if (!isAdmin) {
        throw new Error('Unauthorized: Admin access required');
    }
 ...
 */
export const searchResourceFiles = async (searchTerm, filters) => {
    let query = supabase
        .from('resource_files')
        .select('*')
        .eq('is_public', true)
 ...
 */
export const getAIUsageStats = async (days = 30) => {
    try {
        ...
        const totalTokensUsed = logs?.reduce((sum, log) => sum + (log.tokens_used || 0), 0) || 0;
        const estimatedCost = logs?.reduce((sum, log) => sum + (parseFloat(log.cost_estimate) || 0), 0) || 0;
 ...
 * Log AI usage
 */
export const logAIUsage = async (requestType, model, tokensUsed, success, errorMessage) => {
    try {
        const { data: { user } } = await supabase.auth.getUser();
 ...
 */
export const togglePaymentExemption = async (userId, isExempt) => {
    const isAdmin = await checkIsAdmin();
    if (!isAdmin) {
        throw new Error('Unauthorized: Admin access required');
    }
    const { error } = await supabase
        .from('profiles')
        .update({ is_payment_exempt: isExempt })
        .eq('id', userId);
    if (error) {
 ...
 */
export const getAllUserLessonCounts = async () => {
    try {
        // Fetch profiles
