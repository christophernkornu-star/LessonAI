...
import { GHANA_CONTEXT, getCurriculumStandard, getSubjectExamples, getDifferentiationStrategy } from "@/data/ghanaContext";
...
function getPhilosophyGuidance(philosophy) {
    const philosophies = {
...
function getDetailLevelGuidance(detailLevel) {
...
function getDetailLevelOverride(detailLevel) {
    const normalizedLevel = normalizeDetailLevel(detailLevel);
    const overrides = {
...
// Helper to log usage
async function logAIUsage(model, requestType, success, tokens = 0, errorMessage) {
    try {
...
async function verifyUserEligibility() {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user)
        return; // Allow anonymous generation if app logic permits, or enforce login here. Assuming logged in.
...
export async function callAIAPI(prompt, systemMessage, numLessons, maxTokens) {
    await verifyUserEligibility();
    // Strictly use DeepSeek API
    return callDeepSeekAPI(prompt, systemMessage, numLessons, maxTokens);
}
async function callDeepSeekAPI(prompt, systemMessage, numLessons, maxTokens) {
...
export async function generateLessonNote(originalData) {
    try {
        ...
        if (data.selectedCurriculumFiles && data.selectedCurriculumFiles.length > 0) {
            ...
            if (curriculumFiles && curriculumFiles.length > 0) {
                const filesWithContent = await Promise.all(curriculumFiles.map(async (file) => {
                    let content = "";
                    if (file.file_path) {
                ...
                curriculumFilesInfo = `\n\n**Reference Curriculum Documents:**\n${filesWithContent.map((file, idx) => `${idx + 1}. ${file.title}${file.description ? ` - ${file.description}` : ''} (${file.file_name})\nCONTENT:\n${file.content.substring(0, 3000)}...`).join('\n\n')}`;
            }
        }
        if (data.selectedResourceFiles && data.selectedResourceFiles.length > 0) {
            ...
            if (resourceFiles && resourceFiles.length > 0) {
                const filesWithContent = await Promise.all(resourceFiles.map(async (file) => {
                    let content = "";
                    if (file.file_path) {
...
export async function extractCurriculumFromText(text) {
    const prompt = `
...
export async function parseCurriculumPaste(text) {
    const prompt = `
...
export async function parseSchemeOfLearning(text) {
    const prompt = `
...
function wrapMathInLatex(text) {
    ...
    const replaceOutsideMath = (input, regex, replacer) => {
        return input
            .split(/(\$\$[\s\S]*?\$\$|\$[^$\n]+\$)/g)
            .map((segment) => {
    ...
    result = replaceOutsideMath(result, /(\d+(?:\.\d+)?\s*[+\-*/]\s*\d+(?:\.\d+)?\s*=\s*\d+(?:\.\d+)?)/g, (match) => `$${match}$`);
    // 3. Exponents: "x^2"
    result = replaceOutsideMath(result, /(\d+|[a-zA-Z])\s*\^\s*(\d+|[a-zA-Z])/g, (match) => `$${match}$`);
    // 4. Fractions: "1/2"
    result = replaceOutsideMath(result, /(?<!\d)(\d+)\/(\d+)(?!\d)/g, (match) => `$${match}$`);
...
function processJsonLessonContent(jsonStr, index, totalLessons) {
    ...
    try {
        ...
        try {
            ...
            if (!repairedStr.trim().endsWith('}'))
                repairedStr += '}';
            if (!repairedStr.trim().startsWith('{'))
                repairedStr = repairedStr.trimStart();
