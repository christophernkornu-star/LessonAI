 ...
 */
export const getLessonsBySubject = async (userId) => {
    try {
        let query = supabase
            .from('lesson_notes')
            .select('subject');
 ...
 */
export const getLessonsByGradeLevel = async (userId) => {
    try {
        let query = supabase
            .from('lesson_notes')
            .select('grade_level');
 ...
 */
export const getLessonsByTemplate = async (userId) => {
    try {
        let query = supabase
            .from('lesson_notes')
            .select(`
 ...
 */
export const getLessonsByStrand = async (userId, subject) => {
    try {
        let query = supabase
            .from('lesson_notes')
 ...
 * Get quality metrics
 */
export const getQualityMetrics = async (userId) => {
    try {
        let query = supabase
            .from('lesson_notes')
            .select('*');
        ...
        const favorites_count = data?.filter((l) => l.is_favorite).length || 0;
        const favorite_rate = total_lessons > 0 ? (favorites_count / total_lessons) * 100 : 0;
        const total_content_length = data?.reduce((sum, l) => sum + (l.generated_content?.length || 0), 0) || 0;
        const avg_content_length = total_lessons > 0 ? total_content_length / total_lessons : 0;
        const lessons_with_resources = data?.filter((l) => l.exemplars && l.exemplars.trim().length > 0).length || 0;
        return {
            total_lessons,
            favorites_count,
...
export const getEngagementMetrics = async (userId) => {
    try {
        ...
        const lessons_this_week = data?.filter((l) => new Date(l.created_at) >= weekAgo).length || 0;
        const lessons_this_month = data?.filter((l) => new Date(l.created_at) >= monthAgo).length || 0;
        // Calculate streak
        ...
        return {
            ...
            longest_streak,
            most_active_day,
            most_active_hour: parseInt(most_active_hour),
        };
    }
    catch (error) {
 ...
 */
const calculateStreaks = (lessons) => {
 ...
 */
export const getCurriculumCoverage = async (userId, subject) => {
    try {
        ...
        if (lessonError)
            throw lessonError;
        const uniqueStrands = new Set(curriculumData?.map((c) => c.strand));
        const uniqueSubStrands = new Set(curriculumData?.map((c) => c.sub_strand));
        const coveredStrands = new Set(lessonData?.map((l) => l.strand));
        const coveredSubStrands = new Set(lessonData?.map((l) => l.sub_strand));
 ...
 */
export const getUserActivityData = async (limit = 10) => {
    try {
        ...
        const userActivity = await Promise.all(profiles.map(async (profile) => {
...
export const getUserAchievements = async (userId) => {
    try {
        ...
        const favorites = lessons?.filter((l) => l.is_favorite).length || 0;
...
export const getInsights = async (userId) => {
    try {
        ...
        const recentLessons = lessons?.filter((l) => new Date(l.created_at) >= weekAgo).length || 0;
