...
export class CurriculumService {
    ...
    static async getAllCurriculum(userId) {
    ...
    static async getCurriculumByGradeAndSubject(gradeLevel, subject, userId) {
    ...
    static async getStrandsByGradeAndSubject(gradeLevel, subject, userId) {
        try {
    ...
    static async getSubStrandsByStrand(gradeLevel, subject, strand, userId) {
        try {
            const curriculum = await this.getCurriculumByGradeAndSubject(gradeLevel, subject, userId);
    ...
    static async getContentStandardsBySubStrand(gradeLevel, subject, strand, subStrand, userId) {
        try {
            ...
            curriculum.forEach((c) => {
                ...
                if (strandMatch && subStrandMatch) {
                    if (c.content_standards && Array.isArray(c.content_standards)) {
                        c.content_standards.forEach((standard, index) => {
                            ...
                            if (existingIndex >= 0) {
                                ...
                                (c.learning_indicators || []).forEach((ind) => {
                                ...
                                exemplarsList.forEach(ex => {
                                    if (!newExemplars.includes(ex))
                                        newExemplars.push(ex);
                                });
                                // Add new mapping
    ...
    // Add curriculum
    static async addCurriculum(curriculum, userId) {
        try {
    ...
    static async bulkUploadCurriculum(curriculumList, userId) {
        try {
    ...
    // Update curriculum
    static async updateCurriculum(id, updates) {
        try {
    ...
    static async deleteAllUserCurriculum(userId) {
        try {
            const { error } = await supabase
                .from("curriculum")
                .delete()
    ...
    static async getUniqueSubjects(userId) {
        try {
    ...
    static async getUniqueGradeLevels(userId) {
        try {
    ...
    static async getSubjectsByGradeLevel(gradeLevel, userId) {
        try {
            ...
            data?.forEach((item) => {
                ...
                const matches = (itemLevel === normalizedInput ||
    ...
    static async findRelatedExemplars(userId, gradeLevel, subject, contentStandardText) {
        try {
            ...
            for (const item of curriculumItems) {
                ...
                const isMatch = standards.some((s) => s && s.includes(searchCode));
                if (isMatch) {
...
Object.defineProperty(CurriculumService, "cache", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: new Map()
});
Object.defineProperty(CurriculumService, "CACHE_TTL", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: 5 * 60 * 1000
