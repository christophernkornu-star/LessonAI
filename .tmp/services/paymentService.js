...
export function detectProvider(phoneNumber) {
    ...
    const prefix = cleaned.startsWith('233')
        ? '0' + cleaned.slice(3, 5)
        : cleaned.slice(0, 3);
    return MOBILE_MONEY_PROVIDERS.find(p => p.prefixes.some(pre => prefix.startsWith(pre.slice(0, 3)))) || null;
}
/**
 ...
 */
export async function calculateCost(tokens) {
 ...
 */
export async function estimateLessonCost(numLessons = 1) {
 ...
 */
export async function getUserPaymentProfile() {
    ...
    const { data: profile } = await supabase
        ...
        .eq('id', user.id)
        .single();
    const isProfileExempt = profile?.is_payment_exempt === true;
    const { data, error } = await supabase
 ...
 */
export async function initializePayment(amount, phoneNumber, provider, metadata) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
 ...
 */
export async function processMobileMoneyPayment(amount, phoneNumber, provider, email, reference) {
    try {
        const formattedPhone = formatPhoneNumber(phoneNumber);
 ...
 */
export async function deductPayment(tokens, generationType = 'lesson_note', numLessons = 1) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
        return { success: false, error: 'User not authenticated' };
    }
 ...
 */
export async function setPaymentExemption(targetUserId, exempt, reason) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
 ...
 */
export async function getAllUserPaymentProfiles() {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user)
        return [];
    // Check admin
 ...
 */
export async function addFundsToWallet(targetUserId, amount, reason) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
        return { success: false, error: 'Not authenticated' };
    }
