...
export async function exportToPDF(content, filename = "lesson-note.pdf") {
    exportToPDFInternal(content, filename);
}
export async function exportGhanaLessonToPDF(data, filename = "ghana-lesson-note.pdf") {
...
function exportToPDFInternal(content, filename, isHtml = false) {
...
function generateGhanaLessonHTML(data) {
    const markdownToHtml = (text) => {
        ...
        const processedLines = lines.map(line => {
            ...
            if (!trimmed)
                return '<br/>'; // Maintain empty lines
            ...
            if (isHeader)
                return `<div class="content-header">${innerHtml}</div>`;
            if (isListItem)
                return `<div class="list-item"><span class="bullet">•</span><span class="list-text">${innerHtml}</span></div>`;
