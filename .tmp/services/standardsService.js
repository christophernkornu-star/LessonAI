...
export const standardsService = {
     ...
     */
    async getCoverageStats(subject, level) {
        try {
     ...
     */
    async updateStandardCoverage(id, updates) {
        try {
     ...
     */
    async deleteStandardCoverage(id) {
        try {
            const { error } = await supabase
                .from('standards_coverage')
                .delete()
     ...
     */
    async isStandardCovered(subject, level, contentStandard) {
        try {
            const { data: { user } } = await supabase.auth.getUser();
            if (!user)
                return false;
