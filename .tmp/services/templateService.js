...
export class TemplateService {
    ...
    static async getTemplatesByCurriculum(curriculum) {
        const allTemplates = await this.getAllTemplates();
        return allTemplates.filter(template => template.curriculum === "all" || template.curriculum === curriculum);
    }
    // Add custom template (user-created)
    static async addCustomTemplate(template, userId) {
        try {
            const { data, error } = await supabase
                .from("templates")
                .insert({
