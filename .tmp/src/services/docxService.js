import { Document, Paragraph, TextRun, HeadingLevel, AlignmentType, Packer, Table, TableCell, TableRow, WidthType, BorderStyle, PageBreak, BuilderElement, Math as DocxMath, MathRun as DocxMathRun, MathFraction, MathSuperScript, MathSubScript, MathRadical, createMathAccentCharacter, createMathBase } from "docx";
 ...
 */
export async function generateLessonNoteDocx(lessonNoteText, metadata, fileName = "lesson-note.docx") {
    try {
        ...
        if (tableRows.length > 0) {
            const table = new Table({
                rows: tableRows.map((rowCells) => new TableRow({
