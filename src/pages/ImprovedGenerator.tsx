...
const ImprovedGenerator = () => {
  ...
  } = useDraft<LessonData>(defaultLessonData, {
  ...
  const [availableContentStandards, setAvailableContentStandards] = useState<
    Array<{
      ...
      mappings?: Array<{ indicators: string[]; exemplars: string[] }>;
    }>
  >([]);
  ...
  const [availableLevels, setAvailableLevels] =
    useState<Array<{ value: string; label: string }>>(CLASS_LEVELS);
    ...
    const fetchTimetableDetails = async () => {
      ...
      try {
        ...
        if (!allTimetables || allTimetables.length === 0) {
          ...
          const selectedLevelObj = availableLevels.find(
            (l) => l.value === lessonData.level || l.label === lessonData.level,
          );
          const selectedLevelLabel =
            selectedLevelObj?.label || lessonData.level;

          if (userProfile?.class_sizes) {
            const specificSize =
              userProfile.class_sizes[lessonData.level] ||
        ...
        if (timetable) {
          ...
          if (lessonData.subject && timetable.subject_config) {
            ...
            if (!configKey) {
              configKey = Object.keys(timetable.subject_config).find((k) => {
                ...
                if (
                  (keyLower === "maths" && targetSubject === "mathematics") ||
            ...
            if (configKey) {
              ...
              if (config && config.frequency) {
                ...
                if (
                  lessonData.numLessons === undefined ||
          ...
          console.log(
            "No matching timetable found in list of",
    ...
    const loadMetadata = async () => {
      ...
      if (currentUser) {
        try {
          ...
          try {
            const dbLevels = await CurriculumService.getUniqueGradeLevels(
              currentUser.id,
            );
            ...
            console.warn(
              "Could not load levels from CurriculumService",
              dbError,
            );
          }
          ...
          try {
            ...
            if (timetables && timetables.length > 0) {
              timetables.forEach((tt) => {
                ...
                const staticMatch = CLASS_LEVELS.find(
                  (l) => l.label === ttLevel || l.value === ttLevel,
                );
            ...
            console.warn(
              "Could not load levels from TimetableService",
              ttError,
            );
          }
          ...
          try {
            ...
            if (schemes && schemes.length > 0) {
              schemes.forEach((s) => {
                ...
                if (sLevel) {
                  const staticMatch = CLASS_LEVELS.find(
                    (l) => l.label === sLevel || l.value === sLevel,
                  );
                  if (staticMatch) {
                    levelMap.set(staticMatch.label, staticMatch);
      ...
      if (lessonData.level) {
        const matchLabel = finalLevels.find(
          (l) => l.label === lessonData.level,
        );
        const matchValue = finalLevels.find(
          (l) => l.value === lessonData.level,
        );

        if (matchLabel && !matchValue) {
          console.log(
            "Fixing mismatch: converting Label to Value",
            lessonData.level,
            "->",
            matchLabel.value,
          );
  ...
  useEffect(() => {
    const loadSubjects = async () => {
      if (lessonData.level) {
        ...
        if (currentUser) {
          try {
            const dbSubjects = await CurriculumService.getSubjectsByGradeLevel(
              selectedLevelLabel,
              currentUser.id,
            );
        ...
        if (
          ...
          !subjects.some(
            (s) =>
              s.value === lessonData.subject || s.label === lessonData.subject,
          )
        ) {
          setLessonData((prev) => ({
            ...prev,
    ...
    const loadStrands = async () => {
      if (lessonData.subject && lessonData.level && currentUser) {
        ...
        if (strands.length === 0) {
          ...
          const allCurriculum = await CurriculumService.getUserCurriculum(
            currentUser.id,
          );
      ...
      if (lessonData.subject && lessonData.level && currentUser) {
        ...
        if (currentStrands.length > 0) {
          ...
          const uniqueSubStrands = Array.from(
            new Map(allSubStrands.map((item) => [item.label, item])).values(),
          );
      ...
      if (lessonData.subject && lessonData.level && currentUser) {
        ...
        if (currentStrands.length > 0 && currentSubStrands.length > 0) {
          ...
          const uniqueStandards = Array.from(
            new Map(allStandards.map((item) => [item.code, item])).values(),
          );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      try {
            ...
            if (timetable.subject_config) {
              ...
              if (!subjectConfig) {
                ...
                Object.keys(timetable.subject_config).forEach((key) => {
                  ...
                  // "History" specific handling
                    ...
                    targetSubject.includes("history") &&
                    k.includes("history")
                  ) {
                    score = 90;
                  }
                ...
                if (bestMatchKey && bestMatchScore > 0) {
                  ...
                  console.log(
                    `Fuzzy matched timetable subject: '${bestMatchKey}' for '${lessonData.subject}' (Score: ${bestMatchScore})`,
                  );
                }
              }
              ...
              if (subjectConfig) {
                ...
                if (
                  ...
                  console.warn(
                    "Timetable entry found but NO DAYS configured:",
                    subjectConfig,
                  );
                }
        ...
        console.warn(
          "Failed to fetch timetable, proceeding with defaults",
          ttError,
        );
      }
      ...
      const dataWithTemplate: LessonData = {
        ...
        selectedCurriculumFiles:
          selectedCurriculumFiles.length > 0
            ? selectedCurriculumFiles
            : undefined,
        selectedResourceFiles:
          selectedResourceFiles.length > 0 ? selectedResourceFiles : undefined,
      };
      ...
      const estimatedTokens = Math.max(
        4000,
        Math.ceil(generatedContent.length / 4) + 1500,
      );
      const paymentResult = await deductPayment(
        estimatedTokens,
        "lesson_note",
        finalNumLessons,
      );
        ...
        try {
          ...
          if (locationString) {
            ...
            toast({
              title: "Location detected",
              description:
                "Coordinates set. You can edit the location name manually.",
            });
          }
        } catch (error) {
  ...
  const handlePasteCurriculum = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Could not parse the text. Please fill the fields manually.",
        variant: "destructive",
      });
    } finally {
  ...
  useEffect(() => {
    ...
    if (selectedIndicators.length > 0 && availableContentStandards.length > 0) {
      ...
      const currentStandards =
        selectedContentStandards.length > 0
      ...
      currentStandards.forEach((stdString) => {
        ...
        const selectedStandard = availableContentStandards.find(
          (cs) =>
          ...
          selectedStandard.mappings.forEach((mapping) => {
            ...
            const indicatorsMatch = mapping.indicators.some((ind) => {
              ...
              return selectedIndicators.some(
                (sel) =>
                  sel.toLowerCase().trim().includes(indNorm) ||
      ...
      if (relevantExemplars.size > 0) {
        ...
        setSelectedExemplars((prev) =>
          prev.filter((ex) => newExemplars.includes(ex)),
        );
      } else {
        // Fallback logic
        if (hasMapping) {
  ...
  useEffect(() => {
    ...
    if (lessonData.contentStandard && availableContentStandards.length > 0) {
      const selected = availableContentStandards.find(
        (cs) =>
      ...
      if (selected) {
        ...
        if (lessonData.indicators) {
          const existing = lessonData.indicators
            .split("\n")
            .map((s) => s.trim());
          const matches = (selected.indicators || []).filter((i) =>
            existing.some((e) => i.includes(e) || e.includes(i)),
          );
        ...
        if (lessonData.exemplars) {
          const existing = lessonData.exemplars
            .split("\n")
            .map((s) => s.trim());
          const matches = (selected.exemplars || []).filter((e) =>
            existing.some((ex) => e.includes(ex) || ex.includes(e)),
          );
  ...
  useEffect(() => {
    if (location.state?.fromScheme && location.state?.schemeData) {
      ...
      if (restoreData.templateName) {
        const t = lessonTemplates.find(
          (lt) => lt.name === restoreData.templateName,
        );
        ...
        <main className="container mx-auto px-4 py-8 sm:py-12 max-w-[80rem] flex-grow">
          <div className="mx-auto max-w-4xl">
            <div className="mb-12 sm:mb-16 text-center px-4 flex flex-col items-center max-w-3xl mx-auto">
              ...
              <div className="mt-8 flex w-full sm:w-auto justify-center">
                <Button
                  variant="outline"
                  onClick={() => navigate("/schemes")}
                  className="gap-2 w-full sm:w-auto rounded-full shadow-sm hover:bg-secondary/10 transition-colors"
                >
            ...
            <Card className="p-4 sm:p-6 lg:p-8 group relative overflow-hidden rounded-2xl border border-secondary/20 bg-background/50 backdrop-blur-sm transition-all shadow-xl">
              <div className="space-y-4 sm:space-y-6">
                ...
                {currentStep === 1 && (
                  <div className="space-y-4 sm:space-y-6 animate-in fade-in-50 duration-500">
                      ...
                      >
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              Paste the Strand, Sub-strand, Content Standard,
                              and Indicators from your document here. AI will
                              try to fill the form for you.
                            </DialogDescription>
                          </DialogHeader>
                          <Textarea
                            value={pastedText}
                    ...
                    <div className="flex items-center space-x-2 bg-muted/30 p-3 rounded-lg border border-border/50">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          setIsManualCurriculum(checked as boolean)
                        }
                      />
                      <Label
                        htmlFor="manual-mode"
                      ...
                      >
                        Manually enter curriculum standards (Strand, Sub-strand,
                        etc.)
                      </Label>
                    </div>

                    <div className="grid gap-4 sm:gap-6">
                      <div className="bg-card p-4 sm:p-5 rounded-xl border shadow-md hover:shadow-lg transition-shadow duration-200 space-y-3">
                        <Label htmlFor="strand" className="text-base font-semibold text-primary">Strands *</Label>
                        {!isManualCurriculum &&
                          availableStrands.length === 0 && (
                            <div className="text-xs text-amber-600 dark:text-amber-400 mb-2 bg-amber-50 dark:bg-amber-950/30 p-2 rounded border border-amber-200 dark:border-amber-800">
                              No strands found? Please select a valid
                              subject/level combination.
                            </div>
                          )}
                        {isManualCurriculum ? (
                          <Textarea
                      ...
                      <div className="bg-card p-4 sm:p-5 rounded-xl border shadow-md hover:shadow-lg transition-shadow duration-200 space-y-3">
                        <Label htmlFor="subStrand" className="text-base font-semibold text-primary">Sub-Strands *</Label>
                        {isManualCurriculum ? (
                          <Textarea
                            id="subStrand"
                    ...
                    <div className="bg-card p-4 sm:p-5 rounded-xl border shadow-md hover:shadow-lg transition-shadow duration-200 space-y-3">
                      <Label htmlFor="contentStandard" className="text-base font-semibold text-primary">
                        Content Standards *
                      </Label>
                            ...
                            <MultiSelectCombobox
                              ...
                              onChange={(newSelection) => {
                                ...
                                newSelection.forEach((val) => {
                                  const selected =
                                    availableContentStandards.find((cs) => {
                                      let label = `${cs.code}: ${cs.description}`;
                                      if (cs.page_reference) {
                                        label += ` (${cs.page_reference})`;
                                      }
                                    ...
                                    if (selected.indicators) {
                                      ...
                                      rawIndicators.forEach((ind) => {
                                        ...
                                        if (matches && matches.length >= 1) {
                                          ...
                                          let cleanInd = ind
                                            .replace(
                                              /Indicator:?\s*By the end of the lesson.*?able to:?/i,
                                              "",
                                            )
                                            .trim();
                                ...
                                if (pageRefs.length > 0) {
                                  ...
                                  const pagePart =
                                    uniquePages.toLowerCase().includes("pg") ||
                    ...
                    <div className="bg-card p-4 sm:p-5 rounded-xl border shadow-md hover:shadow-lg transition-shadow duration-200 space-y-3">
                      <Label htmlFor="indicators" className="text-base font-semibold text-primary flex items-center gap-2">
                        Learning Indicators
                    ...
                    <div className="bg-card p-4 sm:p-5 rounded-xl border shadow-md hover:shadow-lg transition-shadow duration-200 space-y-3">
                      <Label htmlFor="exemplars" className="text-base font-semibold text-primary flex items-center gap-2">
                        Exemplars
                        {selectedExemplars.length > 0 && (
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <Combobox
                          ...
                          onValueChange={(value) =>
                            setLessonData({ ...lessonData, philosophy: value })
                          }
                      ...
                      <div className="space-y-2">
                        ...
                        <Combobox
                          ...
                          onValueChange={(value) =>
                            setLessonData({ ...lessonData, detailLevel: value })
                          }
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setLessonData({
                              ...lessonData,
                              includeDiagrams: checked as boolean,
                            })
                          }
                        />
                        <Label
                        ...
                        >
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p>
                                Add descriptions of diagrams, charts, or
                                illustrations to include in the lesson
                              </p>
                            </TooltipContent>
                          </Tooltip>
                        </Label>
                      ...
                      <p className="text-xs text-muted-foreground ml-6">
                        When enabled, the lesson note will include outlines and
                        descriptions of diagrams that should be drawn or
                  ...
                  <div className="space-y-6 sm:space-y-8 animate-in fade-in-50 duration-500">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        Please review the details below before generating your lesson note.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                      ...
                      <div className="bg-card border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow duration-200">
                        <h4 className="font-semibold text-primary text-sm uppercase tracking-wider mb-4 border-b pb-2">
                          Class Details
                        </h4>
                        <div className="space-y-3">
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                            <span className="text-sm text-muted-foreground">Subject</span>
                            <span className="font-medium text-sm bg-secondary/50 px-2 py-0.5 rounded-md">
                          ...
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                            <span className="text-sm text-muted-foreground">Class Size</span>
                            <span className="font-medium text-sm bg-secondary/50 px-2 py-0.5 rounded-md">
                              {lessonData.classSize || "Not set"}
                            </span>
                          </div>
                          <div className="flex flex-col gap-1 border-t pt-2 mt-1">
                            <span className="text-sm text-muted-foreground">Strand</span>
                            <span className="font-medium text-sm leading-tight text-foreground/90">
                      ...
                      <div className="bg-card border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow duration-200">
                        ...
                        <div className="space-y-3">
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                            <span className="text-sm text-muted-foreground">Teaching Philosophy</span>
                            <span className="font-medium text-sm capitalize bg-primary/10 text-primary px-2 py-0.5 rounded-md">
                          ...
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                            <span className="text-sm text-muted-foreground">Diagram Outlines</span>
                            <span className={`font-medium text-sm px-2 py-0.5 rounded-md ${lessonData.includeDiagrams ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-secondary/50'}`}>
                      ...
                      <div className="bg-card border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow duration-200 md:col-span-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                          <div className="sm:col-span-2 flex flex-col gap-1">
                            <span className="text-sm text-muted-foreground">Content Standard</span>
                            <span className="font-medium text-sm leading-relaxed text-foreground/90 bg-secondary/30 p-2 rounded-lg border border-border/50">
                              {lessonData.contentStandard || "Not set"}
                            </span>
                          </div>
                          <div className="flex flex-col gap-3">
                            <div className="flex flex-col gap-1">
                              <span className="text-sm text-muted-foreground">Curriculum Files</span>
                              <span className="font-medium text-sm flex items-center gap-2">
                                <span className="bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 px-2 py-0.5 rounded-md">
                                  {selectedCurriculumFiles.length} selected
                                </span>
                              </span>
                            </div>
                            <div className="flex flex-col gap-1">
                              <span className="text-sm text-muted-foreground">Layout Template</span>
                              <span className="font-medium text-sm bg-secondary/50 px-2 py-1 rounded-md line-clamp-1" title={selectedTemplate?.name || "Default template"}>
                ...
                {/* Navigation Buttons */}
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-6 mt-6 border-t border-border/40">
                  <Button
                    variant="outline"
                    onClick={prevStep}
                    disabled={currentStep === 0 || isGenerating}
                    className="w-full sm:w-auto order-2 sm:order-1 rounded-full shadow-sm hover:bg-secondary/10"
                  >
                  ...
                  <div className="flex gap-2 w-full sm:w-auto order-1 sm:order-2">
                    {currentStep < STEPS.length - 1 ? (
                      ...
                      <Button
                        ...
                        className="bg-gradient-hero hover:opacity-90 w-full sm:w-auto rounded-full shadow-lg transition-transform hover:scale-[1.02] active:scale-[0.98]"
                      >
                        {isGenerating ? (
                          <>
        ...
        <Dialog open={isPasteDialogOpen} onOpenChange={setIsPasteDialogOpen}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Paste the curriculum data copied from another source. The system
                will attempt to parse and fill the relevant fields.
              </DialogDescription>
            </DialogHeader>
        ...
        <Dialog open={isSchemeDialogOpen} onOpenChange={setIsSchemeDialogOpen}>
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Choose a scheme from the list below to apply its data to the
                form. You can also paste new data directly.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setSchemeItems(
                      JSON.parse(
                        localStorage.getItem("scheme_of_learning_data") || "[]",
                      ),
                    );
                  }}
                  disabled={isGenerating}
                ...
                {filteredSchemeItems.length > 0 ? (
                  <div className="space-y-2 p-2">
                    {filteredSchemeItems.map((item) => (
                      <div key={item.id} className="h-[120px] px-1 py-1">
                        <div
                    ...
                    try {
                      ...
                      if (Array.isArray(parsedData)) {
                        ...
                        toast({
                          title: "Data Imported",
                          description:
                            "New scheme data has been imported successfully.",
                        });
                      } else {
                        toast({
                          title: "Invalid Data",
                          description:
                            "Please ensure the pasted data is in the correct format.",
                          variant: "destructive",
                        });
                      }
                    } catch (error) {
